...
import net.wishwall.rong.model.RongEvent;
...
public class ConversationActivity extends BaseApiActivity implements RongIMClient.RealTimeLocationListener, Handler.Callback {
     ...
     * 在 ConversationActivity 收到消息后，选择进入 DrawerActivity，这样就把 DrawerActivity 激活了，当你读完收到的消息点击 返回键 时，程序会退到
...
//        }
    ...
    private void enterSettingActivity() {
        ...
        if (mConversationType == Conversation.ConversationType.PUBLIC_SERVICE
            ...
            Uri uri = Uri.parse("wish://" + getApplicationInfo().packageName).buildUpon()
...
//                        DemoContext.getInstance().setGroupMap(groupM);
                    ...
                    intent.setClass(ConversationActivity.this, ChatFragmentActivity.class);
